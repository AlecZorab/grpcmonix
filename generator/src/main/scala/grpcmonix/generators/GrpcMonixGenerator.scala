...
import scalapb.compiler.{DescriptorImplicits, FunctionalPrinter, GeneratorParams, StreamType}
...
class GrpcMonixGenerator(val params: GeneratorParams) extends protocbridge.ProtocCodeGenerator {
  def run(requestBytes: Array[Byte]): Array[Byte] = {
    ...
    val innerInstance = inner(new DescriptorImplicits(params, fileDescByName.values.toList))
    request.getFileToGenerateList.asScala.foreach { name =>
      val fileDesc = fileDescByName(name)
      val responseFiles = innerInstance.generateFile(fileDesc)
      responseFiles.foreach(b.addFile)
    }
    b.build.toByteArray
  }

  case class inner(implicits: DescriptorImplicits) {
    import implicits._
    ...
    private[this] def serviceMethodDescriptor(method: MethodDescriptor): PrinterEndo = { printer =>
      ...
      printer
        ...
        .addIndented(
          ...
          s""".setFullMethodName(MethodDescriptor.generateFullMethodName("${method
            .getService.getFullName}", "${method.getName}"))""",
          s".setRequestMarshaller(${marshaller(method.inputType)})",
          s".setResponseMarshaller(${marshaller(method.outputType)})",
          ".build()"
        )
        .outdent
    }
    ...
    private[this] def serviceTraitCompanion(service: ServiceDescriptor, fileDesc: FileDescriptor): PrinterEndo = {
      printer =>
        printer
          ...
          .add(
            s"def javaDescriptor: _root_.com.google.protobuf.Descriptors.ServiceDescriptor  = ${fileDesc.fileDescriptorObject.fullName}.javaDescriptor.getServices().get(0)"
          )
          .add(
            s"def scalaDescriptor: _root_.scalapb.descriptors.ServiceDescriptor = ${fileDesc.fileDescriptorObject.fullName}.scalaDescriptor.services(0)"
          )
          .add(
            s"def bindService(serviceImpl: ${service.getName}, executionContext: scala.concurrent.ExecutionContext): io.grpc.ServerServiceDefinition = ???"
          )
          .newline
    ...
    private[this] def stub(service: ServiceDescriptor): PrinterEndo = { printer =>
      printer
        ...
        .indent
        .print(service.getMethods.asScala) { case (p, m) =>
          p.call(clientMethodImpl(m))
        }
      ...
      method.streamType match {
        ...
        case StreamType.ServerStreaming =>
          printer
            ...
            .indent
            .add(s"override def subscribe(subscriber: SubscriberR[_ >: ${method.outputType.scalaType}]): Unit = {")
            .indent
    ...
    private[this] def javaDescriptor(service: ServiceDescriptor): PrinterEndo = { printer =>
      printer
        ...
        .indent
        .add(s"${service.getFile.fileDescriptorObject.fullName}.javaDescriptor.getServices().get(${service.getIndex})")
        .outdent
    }

    def generateFile(fileDesc: FileDescriptor): Seq[CodeGeneratorResponse.File] = {
      fileDesc.getServices.asScala.toList.map { service =>
        val b = CodeGeneratorResponse.File.newBuilder()

        val objectName = s"${service.name}GrpcMonix"
        ...
        val fp = FunctionalPrinter()
          ...
          .indent
          .print(service.getMethods.asScala) { case (p, m) =>
            p.call(serviceMethodDescriptor(m))
          }
          .newline
